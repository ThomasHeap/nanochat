...
Will also try using rustbpe tokenizer to see if bpe picks up on amino-acid motifs.
"""
...
class ProteinTokenizer:
    ...
    def __call__(self, text, prepend=None, append=None):
        """Allow tokenizer to be called like a function."""
        return self.encode(text, prepend=prepend, append=append)
    
    def encode_special(self, token):
        """Encode a special token string to its ID."""
        if token in self.aa_to_id:
            return self.aa_to_id[token]
    ...
    def decode(self, ids):
        ...
        return ''.join([self.id_to_aa.get(i, '') for i in ids if i > 0])  # Skip <pad>
